...
async function onDeviceReady() {
	...
	window.acode = acode;
	window.dexcode = acode;
	await adRewards.init();
	ensureAceCompatApi();

	if (typeof system !== "undefined" && system?.requestPermission) {
		try {
			...
			console.warn("Permission request skipped in non-mobile runtime:", e);
		}
	}
...
async function loadApp() {
	...
	const $headerTail = (
		<div className="header-tail-wrapper" style={{ display: "inline-flex", alignItems: "center" }}>
			{$menuToggler}
			{$windowControls}
		</div>
	);
