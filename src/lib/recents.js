	...
	addFile(file) {
		...
		try {
			if (typeof window !== "undefined") {
				const bridge = window.dexcodeBridge || window.dexcodeNative;
				if (bridge && typeof bridge.addRecentDocument === "function") {
					bridge.addRecentDocument(file).catch(() => {});
				}
			}
		} catch (e) {
			console.warn("Failed to update OS recent document:", e);
		}
	},
	addFolder(url, opts) {
		if (url.slice(-1) === "/") {
